...
import { Search, Menu, X, User, LogOut, Star, Building, Layers, ArrowRight, HelpCircle, Zap } from "lucide-react";
...
export const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Show sticky search after scrolling past hero (400px on homepage, 100px on other pages)
  ...
  return (
    <>
        ...
        <div className="container-custom">
          <div className="flex items-center justify-between h-20">
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`text-sm font-medium tracking-wide transition-colors duration-200 ${
                    location.pathname === link.path ? 'text-[#00CED1]' : 'text-gray-300 hover:text-white'
                  }`}
            ...
            <div className="flex items-center gap-4">
              {/* Search (hidden when sticky search is shown) */}
              {!showStickySearch && (
                <div ref={searchRef} className="relative hidden md:block">
                  ...
                  <SearchDropdown containerRef={searchRef} />
                </div>
              )}
                  ...
                  <Link
                    ...
                    className={`px-4 py-3 transition-colors ${
                      location.pathname === link.path 
                        ? 'text-[#00CED1] bg-[#00CED1]/10' 
                        : 'text-gray-300 hover:text-white hover:bg-[#1A1D24]'
                    }`}
                  >
                    {link.label}
                  </Link>
      ...
      >
        <div className="bg-[#0F1115]/95 backdrop-blur-md border-b border-[#272A30] py-3">
          <div className="container-custom">
            <div className="flex items-center gap-4">
              {/* Logo Mini */}
              <Link to="/" className="flex items-center gap-2">
                <div className="w-8 h-8 bg-[#00CED1] rounded-sm flex items-center justify-center">
                  <span className="text-black font-bold text-sm">DC</span>
                </div>
              </Link>

              {/* Sticky Search Input */}
              <div ref={stickySearchRef} className="flex-1 max-w-2xl relative">
                <form onSubmit={handleSearch}>
                  <div className="relative">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#00CED1]" />
                    <Zap className="absolute left-10 top-1/2 -translate-y-1/2 w-4 h-4 text-[#00CED1] animate-pulse" />
                    <input
                      type="text"
                      value={searchQuery}
                      onChange={(e) => {
                      ...
                      className="w-full pl-16 pr-24 py-3 bg-[#050505] border border-[#272A30] rounded-sm text-white placeholder-gray-500 focus:border-[#00CED1] focus:outline-none"
                ...
                <SearchDropdown containerRef={stickySearchRef} />
              </div>

              {/* Quick Links */}
              <div className="hidden lg:flex items-center gap-4">
                <Link to="/components" className="text-gray-400 hover:text-white text-sm transition-colors">
                  Components
                </Link>
                <Link to="/suppliers" className="text-gray-400 hover:text-white text-sm transition-colors">
                  Suppliers
                </Link>
